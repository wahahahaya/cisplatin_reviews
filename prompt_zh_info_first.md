...
- 聲稱 chronotherapy 已可臨床導入。
- 聲稱已有 survival benefit。
- 聲稱 evening dosing 對所有人都是最佳。
...
- Cisplatin 仍是 OSCC/HNSCC 治療中的 backbone。
- 它用於多個情境：
...
- 1,2-intrastrand d(GpG) crosslinks 是主要 adduct。
- 1,2-d(ApG) 與 1,3-intrastrand adducts。
- DNA helix distortion。
- 被 DNA damage response machinery 辨識。
...
- Proximal tubular cells，尤其是 S3 segment，特別脆弱。
- OCT2-mediated uptake 促成 platinum accumulation。
- 損傷機制：
  - DNA damage，
  - ROS，
...
- Cochlear outer hair cells 與 stria vascularis 特別脆弱。
- OCT2-mediated accumulation 具有機制相關性。
- 損傷由 oxidative stress、mitochondrial dysfunction 與 hair-cell apoptosis 驅動。
- 通常不可逆。
- 在 survivorship 中非常重要。

### 4.3 Oral and gastrointestinal mucositis

重點：

- 在 OSCC/HNSCC 特別重要，因為 radiotherapy 與 cisplatin 會互相加成。
- 快速增生的 mucosal basal epithelium 特別脆弱。
- 損傷機制：
  - DNA damage，
  - ROS，
...
- Dorsal root ganglia 特別脆弱，原因是 platinum accumulation 以及 limited blood-nerve barrier protection。
- 損傷機制：
  - mitochondrial dysfunction，
  - oxidative stress，
  - axonal injury，
  - sensory neuron apoptosis。
- 是長期 survivorship issue。

### 4.6 Myelosuppression

重點：

- 對 cisplatin 而言，比 mucositis 與 nephrotoxicity 次要，但在 combination regimens 中仍相關。
...
- XPA 是 nucleotide excision repair 的核心因子。
- NER 移除 bulky platinum-DNA adducts。
- XPA oscillation 是 circadian biology 與 cisplatin DNA damage repair 之間最強的機制連結。
...
- OCT2 是 nephrotoxicity 與 ototoxicity 的核心。
...
- 除非非常必要，否則不要超過 5 個 display items。
- 不要放純 cisplatin chemical structure figure，除非整合到 organ-specific toxicity 中。
- 除非有高品質資料來源，否則不要做 country-by-country table。
...
撰寫 future trial design section。

讓 proposal 具體。

## Step 7

最後撰寫 introduction 與 conclusion。
